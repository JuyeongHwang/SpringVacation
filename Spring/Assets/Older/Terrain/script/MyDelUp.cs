...
public class MyDelUp : MonoBehaviour
{
    ...
    public virtual void Generate(List<Vertex> upEdge, Dictionary<double, float> hashElev)
    {
        osub.seg = null;
        //init 
        max_x = xsize;
        ...
        {
            polygon.Add(new Vertex(Random.Range(0.0f, xsize-1.0f), Random.Range(0.0f, ysize)));
        }
        ...
        foreach (Vertex vert in mesh.Vertices)
        {
            ...
            if (hashElev.ContainsKey(vert.y))
            {
                elevations.Add(hashElev[vert.y]);
            }
            else
            {
